...
if (isset($_POST['add_temp'])) {
    ...
    if ($jumlah > 0) {
        $insertTemp = mysqli_query($con, "INSERT INTO temp_keluar (idPetugas, idBarang, jumlah, devisi) 
...
if (isset($_POST['submit_all'])) {
    $tempData = mysqli_query($con, "SELECT * FROM temp_keluar WHERE devisi = '$divisi'");
    ...
    while ($row = mysqli_fetch_assoc($tempData)) {
        ...
        $insert = mysqli_query($con, "INSERT INTO keluar (idPetugas, idBarang, jumlah, devisi) 
                                      VALUES ('$idPetugas', '$idBarang', '$jumlah', '$divisi')");
    ...
    mysqli_query($con, "DELETE FROM temp_keluar WHERE devisi = '$divisi'");
    echo "<script>
        Swal.fire({
            icon: 'success',
            title: 'Berhasil',
                    ...
                    <div class="col-md-6">
                        <div class="card">
                            <div class="card-header">
                                <h5>Form Input Barang Keluar</h5>
                            </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-block">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <?php
                                        $tempData = mysqli_query($con, "SELECT temp_keluar.*, petugas.nama AS petugas, barang.nama AS barang 
                                                                        FROM temp_keluar 
                                                                        JOIN petugas ON temp_keluar.idPetugas = petugas.id 
                                                                        JOIN barang ON temp_keluar.idBarang = barang.id 
                                                                        WHERE temp_keluar.devisi = '$divisi'");
                                        $no = 1;
                                        while ($row = mysqli_fetch_assoc($tempData)) {
                                            echo "<tr>
                                                    <td>{$no}</td>
                                                    ...
                                                    <td>{$row['jumlah']}</td>
                                                    <td>
                                                        <a href='?page=hapusKeluar&id={$row['id']}' class='btn btn-outline-danger btn-sm'>Hapus</a>
                                                    </td>
                                                </tr>";
                                            $no++;
                                        }
